...
class Config(BaseModel):
    ...
    result_db_configs: conlist(ResultDbConfig, min_items=1)
    compute_db_configs: conlist(ResultDbConfig, min_items=1)
...
def parse_config_file(path: Path) -> Config:
    ...
    for name, config in raw_config[RESULT_DB].items():
        result_db_configs.append(ResultDbConfig(name=name, **config))
    ...
    for name, config in raw_config[COMPUTE_DB].items():
        compute_db_configs.append(ComputeDbConfig(name=name, **config))
